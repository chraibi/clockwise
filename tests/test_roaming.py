...
def test_free_space_is_unbiased_for_a_biased_agent():
    # the bias acts only at the wall; at the centre a biased agent's mean heading change is ~0
    cfg = ArenaConfig(wander_sigma=0.3)
    rng = random.Random(1)
    r = Roamer(heading=0.0, biased=True)
    deltas = []
    for _ in range(5000):
        before = r.heading
...
def test_biased_agent_turns_left_when_facing_the_wall():
    # at the +x rim, heading pointing outward (+x), a biased agent turns LEFT (heading increases)
...
def test_control_agent_turns_toward_centre_at_the_wall():
    # an unbiased agent uses the symmetric inward turn. Heading pointing tangentially (+y)
